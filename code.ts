...
interface BionicSettings {
  fixationStrength: number;  // Controls how many characters to bold (1-100%)
  contrast: number;          // Controls the weight difference (100-900)
}
        ...
        if (node.type === 'TEXT') {
          try {
            ...
            if (fontNames === figma.mixed) {
              ...
              await processMixedFontsNode(node, settings);
            } else {
              ...
              const availableStyles = await getAvailableFontStyles(regularFont.family);
              ...
              try {
                ...
                await figma.loadFontAsync({ family: regularFont.family, style: baseStyle });
                await figma.loadFontAsync({ family: regularFont.family, style: boldStyle });
                ...
                if (availableStyles.length > 2) {
                  try {
                    ...
                    const potentialWeights = availableStyles.filter(style => 
                      style.toLowerCase().indexOf('regular') !== -1 ||
                    ...
                    for (const weight of potentialWeights) {
                      if (weight !== baseStyle && weight !== boldStyle) {
                        await figma.loadFontAsync({ family: regularFont.family, style: weight });
                      }
                    }
                  } catch (e) {
...
async function analyzeAvailableWeights() {
  const selection = figma.currentPage.selection;
    ...
    for (const node of selection) {
      if (node.type === 'TEXT') {
        try {
          ...
          if (fontNames !== figma.mixed) {
            const availableStyles = await getAvailableFontStyles(fontNames.family);
            ...
            for (const pos of samplePositions) {
              if (pos < characters.length) {
                ...
                if (fontAtPosition !== figma.mixed) {
                  const availableStyles = await getAvailableFontStyles(fontAtPosition.family);
                  maxWeights = Math.max(maxWeights, availableStyles.length);
                }
              }
            }
  ...
  if (maxWeights > 3) {
    ...
    if (contrastSteps.indexOf(100) === -1) contrastSteps.unshift(100);
    if (contrastSteps.indexOf(450) === -1) contrastSteps.push(450);
    if (contrastSteps.indexOf(900) === -1) contrastSteps.push(900);
...
async function getAvailableFontStyles(fontFamily: string): Promise<string[]> {
...
function mapWeightToNumber(weightName: string): number {
...
function findLightestWeight(availableStyles: string[], currentStyle: string): string {
...
function findBoldWeightWithContrast(availableStyles: string[], contrast: number, baseStyle: string): string {
...
function convertToBionic(textNode: TextNode, settings: BionicSettings, boldStyle: string, baseStyle: string): void {
...
async function processMixedFontsNode(textNode: TextNode, settings: BionicSettings): Promise<void> {
  const characters = textNode.characters;
  const words = characters.split(/\s+/);
    ...
    try {
      ...
      if (fontAtPosition !== figma.mixed) {
        if (!fontFamilies.has(fontAtPosition.family)) {
          ...
          const styles = await getAvailableFontStyles(fontAtPosition.family);
          if (styles.length >= 2) {
  ...
  const resetPositions: {start: number, end: number, family: string, baseStyle: string, boldStyle: string}[] = [];
  ...
  for (const position of resetPositions) {
    try {
      await figma.loadFontAsync({ family: position.family, style: position.baseStyle });
  ...
  for (const position of resetPositions) {
    try {
      ...
      await figma.loadFontAsync({ family: position.family, style: position.boldStyle });
